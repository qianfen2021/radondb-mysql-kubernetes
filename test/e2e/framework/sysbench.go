...
func (f Framework) PrepareData(mysqlcluster *apiv1alpha1.MysqlCluster, options *SysbenchOptions) {
...
func (f Framework) RunOltpTest(mysqlcluster *apiv1alpha1.MysqlCluster, options *SysbenchOptions) {
	name := "sysbench-run"
	cmd := options.cmd(fmt.Sprintf("%s-leader", mysqlcluster.Name))
	cmd = fmt.Sprintf(`while true; \
		do %s \
		run; \
		done`, cmd)
	args := []string{cmd}
